    ...
    if (this != serverPlayer) {
      if (newTick < this._tick) {
        if (!warnedAboutNegativeTicks) {
          console.warn(
            `Reducing player tick from ${this._tick} to ${newTick} - position/inventory information can no longer be guanteed valid`
          );
        }
      } else {
...
const doTickUpdate = (player, newTick) => {
  ...
  if (-1 != player.cursorSlot) {
    ...
    if (currentCursorStack != player.cursorStack) {
      console.warn(
        `cursorStack/cursorSlot mismatch!? cursorStack claims ${player.cursorStack} but cursorSlot (${player.cursorSlot}) claims ${currentCursorStack}`
      );
    }
  }
...
globalObject.Player.prototype.schedule = function (tick, action) {
  if (tick < this.tick) {
    console.warn(
      `Can't schedule an action for the past! ${tick} < ${this.tick}`
    );
    return;
  }
  if (tick === this.tick) {
...
globalObject.Player.prototype._stopRunning =
  globalObject.Player.prototype.stopRunning;
...
globalObject.Player.prototype.checkInventory = function (str) {
  ...
  this.print(`"-----------------------actual:"`);
  this.chat(
    '/c for item, count in pairs(game.player.get_inventory(1).get_contents()) do game.player.print(item .. ": " .. count) end'
  );
};

globalObject.Player.prototype.checkPosition = function () {
  this.print(
    `"Checking position - expected: {${this.position[0]}, ${this.position[1]}}"`
  );
  this.print(
    `"----------------------actual: {" .. game.players[${
      this.id + 1
    }].character.position.x .. ", " .. game.players[${
      this.id + 1
  ...
  for (let i = 0; i < 49; i++) {
    ...
    for (let j = -2; j <= 2; j++) {
      for (let k = -2; k <= 2; k++) {
        ...
        const realDeltaX = Math.abs(
          realX - roundToFixed(maybeBlocked[0] * 0.2)
        );
        const realDeltaY = Math.abs(
          realY - roundToFixed(maybeBlocked[1] * 0.2)
        );
...
globalObject.spawnPlayers = (nameList) => {
  ...
  if (nameList.length > startingPositions.length) {
    console.warn(
      `Spawning too many (${nameList.length}) players at once - only have calculated start positions for the first ${startingPositions.length}`
    );
  }
...
globalObject.Player.prototype._moveSelection =
  globalObject.Player.prototype.moveSelection;
...
globalObject.Player.prototype.isSelectedBy = function (pos) {
  ...
  return -0.4 < delta[0] && delta[0] < 0.4 && -1.4 < delta[1] && delta[1] < 0.2;
};
...
globalObject.Player.prototype._dropItem =
...
globalObject.Player.prototype._clearCursor =
  globalObject.Player.prototype.clearCursor;
...
globalObject.Player.prototype.craft = function (recipeName, count) {
  ...
  for (let i = 0; i < ingredients.length; ++i) {
    if (ingredients[i].amount * realCount > invIngredients[i]) {
      console.warn(
        `Don't have enough ${ingredients[i].name} for ${recipeName} (have ${
          invIngredients[i]
        }, need ${ingredients[i].amount * realCount})`
      );
      haveEnough = false;
    }
  }
  ...
  // Schedule the crafts
  const startTick =
    this.craftEndTick <= this.tick ? this.tick : this.craftEndTick;
...
globalObject.Player.prototype.grab = function (item) {
  ...
  console.warn(
    `No ${item} in inventory to grab! Did you forget to explicitly stow it?`
  );
};
